...
The core library (`mujin_core`) is ROS-agnostic. The `ros2/` directory is a separate `ament_cmake` package (ROS2 Jazzy) that wraps the core with three lifecycle nodes.
...
- **ExecutorNode** (`ros2/src/executor_node.cpp`) — owns BT factory + tree. Ticks via `rclcpp::Timer` at 50 Hz. MujinBTLogger publishes to `/executor/bt_events`. Supports direct `WorldModel*` (in-process) or service-backed BT nodes (distributed).
...
- `mujin_core` is completely unchanged — ROS2 is purely an adapter layer

...
- `doc/plans/AME/autonomy_assurance_plan.md:209` defines `SR-08`: `ActionRegistry.resolve()` shall fail loudly if no mapping exists.
...
- `doc/plans/AME/autonomy_assurance_plan.md:169-173` lists replan limit and safe-state fallback as Safe Operating Concept controls.
- `doc/plans/AME/autonomy_assurance_plan.md:207-210` defines `SR-06` and `SR-09` around replanning limits and always-available safe-state behavior.
...
- `doc/plans/AME/autonomy_assurance_plan.md:168` says bounded planning time is implemented via a `max_iterations` parameter in the LAPKT solver.
- `doc/plans/AME/autonomy_assurance_plan.md:203` defines `SR-02`: planner shall terminate within N ms or return `NO_PLAN`.
...
- The core Stage 5 and Stage 7 examples in `doc/assurance/AME/sace-pddl/12-e2e-example.md:144-156` and `doc/assurance/AME/sace-pddl/12-e2e-example.md:183-211` therefore are not backed by CI-visible tests.
...
- The separation between runtime execution and offline assurance tooling is clear and well-motivated in `doc/assurance/AME/sace-pddl/13-tooling-analysis.md:170-219`.

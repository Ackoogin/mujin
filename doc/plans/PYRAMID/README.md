...
| **Live plan** (in progress; Phases 0–1 complete — both drops pinned, converter proven strict-clean on both, P1 tree checked in with reconciliation documented; next: Phase 2 codec-gen generalization) | [`uci_mms_conversion_plan.md`](uci_mms_conversion_plan.md) — `xsd2proto` + profile ladder: from the hand-authored UCI seam test cases to a GRA-scale minimum message set converted from the XSD (rung 3) |

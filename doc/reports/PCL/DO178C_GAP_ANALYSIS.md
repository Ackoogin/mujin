...
|---|---|---|
| High-level requirements | `subprojects/PCL/doc/requirements/HLR.md` (PCL.001-PCL.076, with rationale and design-decision trace) | Present, current |
| Low-level requirements | `subprojects/PCL/doc/requirements/LLR.md` (377 LLRs, REQ_PCL_001-462, each with HLR trace + verification pointer) | Present, current |
...
| Test-to-requirement trace | `///< REQ_PCL_NNN` tags in tests; `doc/reports/PCL/HLR_COVERAGE.md` matrix, generated by `subprojects/PCL/scripts/gen_hlr_coverage.py --check` | Present, current, machine-checked (zero gaps, 2026-07-06) |
| Structural coverage | `doc/reports/PCL/COVERAGE_REPORT.md`: **100%** statement coverage over all 17 current sources (4300/4300 lines, 440/440 tests), branch coverage 73% recorded informationally | Present, current (2026-07-06); statement-only for credit |
...
| GAP-C-08 (code-to-LLR trace) | **Open** — convention decided (function-level tags, coding standard 5.6) but not applied. |
...
| GAP-C-13 (structural coverage) | **Closed on host** — harness extended to all 17 sources + 21 test binaries; 100% statement coverage (4300/4300), 440/440 tests pass (Linux/GCC 13, 2026-07-06). Remains host-only evidence pending GAP-C-11. |
...
| GAP-B-01 (decision coverage) | **Sized** — branch coverage 73% (2429/3285) recorded in `COVERAGE_REPORT.md` as the planning input. |

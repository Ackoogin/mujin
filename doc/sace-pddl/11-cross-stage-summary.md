...
| **3 — Safe Operating Concept** | Trajectory constraints and feasibility analysis | Solution nodes (feasibility proofs) supporting mitigation goal | Known-unsafe mitigation proof | Safety test objective encoding |
| **4 — Safety Requirements** | Decomposed sub-domains and satisfaction proofs | Solution nodes (completeness analysis, satisfaction proofs) supporting decomposition goal | Insufficiency-to-subsystem tracing | Risk factor decomposition alignment |
...
| **7 — Out of Context** | Fallback plan library and reachability analysis | Solution nodes (reachability proofs, fallback plans) supporting safe-state goal | ODD excursion handling | Combined risk factor analysis |
...
| **Residual Risk** | Coverage metrics, fault-injected variant results, misuse scenario analyses, response verification traces | Solution nodes (coverage, response verification) + assumption nodes (model fidelity, insufficiency completeness) + context nodes (acceptance criteria) | Core SOTIF: known-unsafe mitigation evidence and unknown-unsafe coverage bounding | Risk factor completeness checklist and Annex K randomised exploration methodology |
...
| **Unknown-unsafe** | Unexplored PDDL state space (the region to be minimised) |

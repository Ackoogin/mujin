...
| Reconcile the RPC/pub-sub seam duality on 4-rpc Request-shape services | **Planned** — [`rpc_pubsub_interchangeability_plan.md`](../../plans/PYRAMID/rpc_pubsub_interchangeability_plan.md) (2026-07-09) | Every `Create/Read/Update/Cancel` method generates *both* an RPC seam (`invoke*`/`dispatch*`, `consumed`/`provided` kind) and a pub/sub seam (`publish*`/`subscribe*`, `publisher`/`subscriber` kind) with nothing reconciling a manifest that routes both. The plan makes the seams interchangeable realizations of one interaction (transaction-shaped facade, per-leg realization choice at compose time, exclusivity enforced fail-closed). Finding: `agra_pubsub_shm_udp_proving_plan.md`'s Phase C follow-up note. |

...
| Reconcile the RPC/pub-sub seam duality on 4-rpc Request-shape services | Someone builds a deployment that (mis)routes both seams for the same rpc | Every `Create/Read/Update/Cancel` method generates *both* an RPC seam (`invoke*`/`dispatch*`, `consumed`/`provided` kind) and a pub/sub seam (`publish*`/`subscribe*`, `publisher`/`subscriber` kind) with nothing reconciling a manifest that routes both. Ideal shape: either bindings always use service semantics and project to pub/sub at the transport layer, or both are generated as explicitly interchangeable via a single facade method (WS-E's local/remote `configureTransport` pattern, generalized). See `agra_pubsub_shm_udp_proving_plan.md`'s Phase C follow-up note for the finding. |

...
| [`agra_pubsub_shm_udp_proving_plan.md`](../../plans/PYRAMID/agra_pubsub_shm_udp_proving_plan.md) | **Executed 2026-07-08** (phases A–E, plan-terminal): A-GRA example contract (`pim/agra_example/`) pub/sub proven over real SHM + UDP transports, including the fail-closed negative gate and a mixed two-transport finale; carried-forward notes tracked here (D-list: routing-manifest-generation-for-real-transports, RPC/pub-sub seam duality; Ada/Windows parity authored not run) |
...
| Opt-in capability adapters (`PUBSUB over RPC_STREAM`, `RPC_UNARY over PUBSUB` for free-form services) | Concrete need | Stay strictly fail-closed until then. The correlated-pair pattern (pub/sub plan §1/§5) is the ready-made spec; contract-derived port topics already cover the MBSE tree. **Annotated per D7 of `rpc_pubsub_interchangeability_plan.md` (resolved 2026-07-09):** for *grammar-conforming* Request/Information ports, "run this interaction over a PUBSUB-only transport" is now just a route-line realization choice (no adapter) — that plan's D1/D5. This row now covers *free-form (non-grammar) services only*; unchanged and still deferred for them. |
...
| `contract_routing_manifest.py` support for real (non-stub) transports | A generated manifest needs to target SHM/UDP/etc. rather than `contract_transport_plugin.c` | Today it only emits `{"mode":"rpc"\|"pubsub"}` config for the NULL-vtable stub. Needs `bus_name`/`participant_id` (SHM) and `remote_host`/`remote_port`/`local_port`/`peer_id` (UDP) config emission, plus the counterpart-participant-id peer-alias convention `agra_pubsub_shm_udp_proving_plan.md` Phase A found is load-bearing for shared-bus transports (not needed for point-to-point ones like UDP). `agra_pubsub_shm_udp_proving_plan.md`'s Phases A/C/D/E hand-author their manifests instead. |
| Reconcile the RPC/pub-sub seam duality on 4-rpc Request-shape services | **Resolved** — [`rpc_pubsub_interchangeability_plan.md`](../../plans/PYRAMID/rpc_pubsub_interchangeability_plan.md) (Phases 0-5 executed 2026-07-09) | Every `Create/Read/Update/Cancel` method generates *both* an RPC seam (`invoke*`/`dispatch*`, `consumed`/`provided` kind) and a pub/sub seam (`publish*`/`subscribe*`, `publisher`/`subscriber` kind); the plan made them interchangeable realizations of one interaction (transaction-shaped C++ facade — `RequestPortClient`/`RequestPortProvider`/`InformationPortSink`/`InformationPortSource` — per-leg realization choice at compose time via a manifest `exclusive` group, fail-closed against dual-routing). Phase 5's terminal harness (`agra_seam_interchange_test`) proved rpc/rpc, pubsub/pubsub, and mixed rpc+pubsub all work cross-process over real shared memory with byte-identical component objects, and along the way found and fixed two real PCL-level gaps (manifest-driven streaming invoke never dispatched remotely; a `provided`-kind endpoint's inbound requests were silently dropped without the transport's gateway container wired in — see `PCL.078`/`REQ_PCL_470`/`471`). Ada parity is spec-only (Phase 4's own recorded fallback — no GNAT/gprbuild in the working environment to compile-verify a full runtime port). Free-form (non-grammar) services are unaffected (D7, see the capability-adapters row above). Finding origin: `agra_pubsub_shm_udp_proving_plan.md`'s Phase C follow-up note. |

...
PlannerNode::on_activate(const rclcpp_lifecycle::State&) {
    ...
    pub_bt_xml_ = create_publisher<std_msgs::msg::String>(
...
void PlannerNode::executePlan(std::shared_ptr<GoalHandlePlan> goal_handle) {
    ...
    if (pub_bt_xml_ && pub_bt_xml_->is_activated()) {
        ...
        msg.data = execution_result.bt_xml;
        pub_bt_xml_->publish(msg);
    }

    RCLCPP_INFO(get_logger(), "Plan found: %zu steps, %.1f ms",
                execution_result.plan_actions.size(), execution_result.solve_time_ms);

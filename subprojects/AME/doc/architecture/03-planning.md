...
The analysis itself is not the tool's own. Finding the context predicates, enumerating their combinations, and carrying a conclusion from one combination to another all live in `ame_core` as `ContingencySearch` (`include/ame/contingency_search.h`). Three things ask the same question of a domain — this tool from the command line, the authoring tool over a project being edited, and the generated assurance report — and sharing one implementation is what stops them answering it differently. The verifier keeps its own presentation: the report layout, the JSON schema, the pruning justification it prints, and the reachable-state expansion behind `--expansion-sidecar`. `ContingencySearch` refuses to carry conclusions when a domain has negative preconditions, because adding a fact can then take an action away, and the reasoning the pruning depends on no longer holds.

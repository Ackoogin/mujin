...
The suite covers: WorldModel, TypeSystem, ActionRegistry, PlanCompiler, Planner, PDDL Parser, BT integration, end-to-end pipeline, observability (Layers 1-5), PCL lifecycle/executor/transport, tactical objects component/codec/streaming, Ada generated bindings, cross-language E2E (Ada <-> C++ over socket), and multi-codec dispatch (JSON, FlatBuffers, Protobuf). The Ada active-find E2E bridge test skips gracefully when its binary is not built. Use `ctest --test-dir build -N` to list the exact set in your build tree.

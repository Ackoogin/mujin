...
- `subprojects/AME/docs/sace-pddl/`
- `subprojects/AME/domains/uav_search_sace/`
- `src/plan_compiler.cpp`
- `src/planner.cpp`
- `subprojects/AME/ros2/src/nodes/planner_node.cpp`
- `subprojects/AME/ros2/src/nodes/executor_node.cpp`
- `subprojects/AME/ros2/src/apps/combined_main.cpp`
...
- `subprojects/AME/docs/autonomy_assurance_plan.md:209` defines `SR-08`: `ActionRegistry.resolve()` shall fail loudly if no mapping exists.
- `subprojects/AME/tests/test_action_registry.cpp:53-56` verifies that `ActionRegistry::resolve()` throws for unknown actions, but there is no matching `PlanCompiler` test for this path.
...
- `subprojects/AME/domains/uav_search_sace/domain_baseline.pddl:77-80` defines `loiter-in-place`.
- `subprojects/AME/domains/uav_search_sace/domain_degraded.pddl:79-109` defines `emergency-return-gps`, `emergency-return-inertial`, and `loiter-in-place`.
- The demo/runtime wiring only registers `move`, `search`, and `classify` in `subprojects/AME/src/apps/main.cpp:162-165` and `subprojects/AME/ros2/src/apps/combined_main.cpp:93-95`.
...
- `subprojects/AME/ros2/src/nodes/planner_node.cpp:162-196` compiles `bt_xml` and returns it in the action result.
- `subprojects/AME/ros2/src/nodes/executor_node.cpp:85-133` only starts execution when `loadAndExecute(bt_xml)` is called.
- `subprojects/AME/ros2/test/test_full_pipeline.cpp:150-159` performs that call manually in the test harness.
- `subprojects/AME/ros2/src/apps/combined_main.cpp:13-18` states that after sending a `/ame/plan` goal, the executor will tick the compiled BT automatically, but no such handoff exists in the production code shown above.
...
- `subprojects/AME/docs/autonomy_assurance_plan.md:169-173` lists replan limit and safe-state fallback as Safe Operating Concept controls.
- `subprojects/AME/docs/autonomy_assurance_plan.md:207-210` defines `SR-06` and `SR-09` around replanning limits and always-available safe-state behavior.
- `doc/concept.md:31` describes a `MissionExecutor` with replan-on-failure.
- `subprojects/AME/ros2/action/Plan.action:3` exposes a `replan` flag.
- `subprojects/AME/ros2/src/nodes/planner_node.cpp:79-197` never reads `goal->replan`.
- Repository search found no implementation of `MissionExecutor` or `ReplanOnFailure` in `src/` or `subprojects/AME/ros2/`.
...
- `subprojects/AME/docs/autonomy_assurance_plan.md:168` says bounded planning time is implemented via a `max_iterations` parameter in the LAPKT solver.
- `subprojects/AME/docs/autonomy_assurance_plan.md:203` defines `SR-02`: planner shall terminate within N ms or return `NO_PLAN`.
...
- `subprojects/AME/tests/test_pddl_parser.cpp:183-193` only parses `subprojects/AME/domains/uav_search/domain.pddl` and `subprojects/AME/domains/uav_search/problem.pddl`.
- Repository search found no references to `uav_search_sace` under `subprojects/AME/tests/` or `subprojects/AME/ros2/test/`.
- The core Stage 5 and Stage 7 examples in `subprojects/AME/docs/sace-pddl/12-e2e-example.md:144-156` and `subprojects/AME/docs/sace-pddl/12-e2e-example.md:183-211` therefore are not backed by CI-visible tests.
...
- The separation between runtime execution and offline assurance tooling is clear and well-motivated in `subprojects/AME/docs/sace-pddl/13-tooling-analysis.md:170-219`.
- The SACE document set is concrete, not abstract; it ties each stage to specific PDDL artefacts in `subprojects/AME/domains/uav_search_sace/`.

...
set "DOMAIN_PATH=%REPO_ROOT%\subprojects\AME\domains\uav_search\domain.pddl"
set "PROBLEM_PATH=%REPO_ROOT%\subprojects\AME\domains\uav_search\problem.pddl"
...
if not exist "%REPO_ROOT%\build\python\_ame_py.!PY_ABI!-win_amd64.pyd" (
    echo.
...
cd /d "%REPO_ROOT%"
"%PYTHON%" -m subprojects.AME.tools.devenv --backend pcl --domain "%DOMAIN_PATH%" --problem "%PROBLEM_PATH%"

...
| DR-08 | PCL.064–PCL.070 | Plugin ABI, loader, and manifest routing exist to support composable deployments | Two certified configurations exist (PSAC 3.3.1): static registration (loader deactivated), or bounded dynamic composition where the deployment's enumerated plugins are verified CC1 items, the manifest is certified configuration data, loading is init-time only, and the loader verifies binary integrity (integrity check not yet implemented). Loaders fail closed either way. |

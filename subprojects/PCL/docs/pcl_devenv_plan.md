...
Enable `subprojects/AME/tools/devenv` to run without ROS2 by providing Python bindings directly to the PCL-level components (`WorldModelComponent`, `PlannerComponent`, `ExecutorComponent`).
...
### Step 1: Python Bindings (`subprojects/AME/src/bindings/`)

Create pybind11 bindings for core components:

```cpp
// subprojects/AME/src/bindings/ame_py.cpp
...
```cmake
# subprojects/AME/src/CMakeLists.txt (additions)
...
### Step 3: PCL Client (`subprojects/AME/tools/devenv/comms/pcl_client.py`)
...
```python
# subprojects/AME/tools/devenv/config.py
...
```python
# subprojects/AME/tools/devenv/main.py
...
python -m subprojects.AME.tools.devenv --backend pcl
```
...
| File | Action | Purpose |
|------|--------|---------|
| `subprojects/AME/src/bindings/ame_py.cpp` | New | pybind11 bindings |
| `subprojects/AME/src/CMakeLists.txt` | Modify | Add Python target |
| `subprojects/AME/tools/devenv/comms/pcl_client.py` | New | Direct PCL client |
| `subprojects/AME/tools/devenv/config.py` | Modify | Add backend option |
| `subprojects/AME/tools/devenv/main.py` | Modify | Backend selection |
| `subprojects/AME/tools/devenv/start_devenv_pcl.bat` | New | PCL-mode launcher |

...
if exist "%CORE_LIB%" if exist "%SOCKET_LIB%" (
...
set "CFLAGS=-std=c11 -O2 -I%ROOT_DIR%\include -I%ROOT_DIR%\src\pcl"

gcc %CFLAGS% -c "%ROOT_DIR%\src\pcl\pcl_container.c" -o "%OBJ_DIR%\pcl_container.o" || exit /b 1
gcc %CFLAGS% -c "%ROOT_DIR%\src\pcl\pcl_executor.c" -o "%OBJ_DIR%\pcl_executor.o" || exit /b 1
gcc %CFLAGS% -c "%ROOT_DIR%\src\pcl\pcl_log.c" -o "%OBJ_DIR%\pcl_log.o" || exit /b 1
gcc %CFLAGS% -c "%ROOT_DIR%\src\pcl\pcl_bridge.c" -o "%OBJ_DIR%\pcl_bridge.o" || exit /b 1
gcc %CFLAGS% -c "%ROOT_DIR%\src\pcl\pcl_transport_socket.c" -o "%OBJ_DIR%\pcl_transport_socket.o" || exit /b 1

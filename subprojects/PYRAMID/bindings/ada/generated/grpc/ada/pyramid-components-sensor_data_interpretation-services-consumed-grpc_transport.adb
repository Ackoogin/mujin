...
package body Pyramid.Components.Sensor_data_interpretation.Services.Consumed.GRPC_Transport is
   ...
   function Decode_Object_Evidence_Provision_Requirement_Array (Response_Json : String) return Object_Evidence_Provision_Requirement_Array is
      J : constant JSON_Value := Read (Response_Json);
      Arr : constant JSON_Array := Get (J);
      Len : constant Natural := Length (Arr);
      Result : Object_Evidence_Provision_Requirement_Array (1 .. Len);
   begin
      for I in Result'Range loop
         Result (I) := Pyramid.Data_Model.Sensors.Types_Codec.From_Json (Write (Get (Arr, I)), null);
      end loop;
      return Result;
   end Decode_Object_Evidence_Provision_Requirement_Array;

   function Decode_Object_Aquisition_Requirement_Array (Response_Json : String) return Object_Aquisition_Requirement_Array is
      J : constant JSON_Value := Read (Response_Json);
      Arr : constant JSON_Array := Get (J);
      Len : constant Natural := Length (Arr);
      Result : Object_Aquisition_Requirement_Array (1 .. Len);
   begin
      for I in Result'Range loop
         Result (I) := Pyramid.Data_Model.Sensors.Types_Codec.From_Json (Write (Get (Arr, I)), null);
      end loop;

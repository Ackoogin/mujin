...
package body Pyramid.Components.Sensor_data_interpretation.Services.Provided.GRPC_Transport is
   ...
   function Decode_Capability_Array (Response_Json : String) return Capability_Array is
      J : constant JSON_Value := Read (Response_Json);
      Arr : constant JSON_Array := Get (J);
      Len : constant Natural := Length (Arr);
      Result : Capability_Array (1 .. Len);
   begin
      for I in Result'Range loop
         Result (I) := Pyramid.Data_Model.Common.Types_Codec.From_Json (Write (Get (Arr, I)), null);
      end loop;
      return Result;
   end Decode_Capability_Array;

   function Decode_Interpretation_Requirement_Array (Response_Json : String) return Interpretation_Requirement_Array is
      J : constant JSON_Value := Read (Response_Json);
      Arr : constant JSON_Array := Get (J);
      Len : constant Natural := Length (Arr);
      Result : Interpretation_Requirement_Array (1 .. Len);
   begin
      for I in Result'Range loop

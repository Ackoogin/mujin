...
public:
    ProvidedComponent(pcl::Executor& executor,
        ...
        : pcl::Component("pyramid_services_sensor_data_interpretation_consumed_provider"),
          executor_(&executor),
          handler_(&handler),
          content_type_(std::move(content_type)),
          bridge_(*this) {}
...
public:
    ConsumedComponent(pcl::Executor& executor,
                      std::string content_type = kJsonContentType)
        : pcl::Component("pyramid_services_sensor_data_interpretation_consumed_consumer"),
          executor_(&executor),
          content_type_(std::move(content_type)) {}
    ...
    std::future<Result<std::vector<ObjectEvidenceProvisionRequirement>>>
    dataProvisionDependencyReadRequirementAsync(const Query& request) {
        auto state = std::make_shared<StreamCollectState<ObjectEvidenceProvisionRequirement>>();
        auto future = state->promise.get_future();
        state->decoder = [](const pcl_msg_t* msg, ObjectEvidenceProvisionRequirement* out) {
            return decodeDataProvisionDependencyReadRequirementStreamFrame(msg, out);
        };
        auto holder = std::make_unique<StreamCollectHolder<ObjectEvidenceProvisionRequirement>>(StreamCollectHolder<ObjectEvidenceProvisionRequirement>{state});
        const pcl_status_t rc = invokeDataProvisionDependencyReadRequirementStream(
            executor_->handle(), request,
            &StreamCollectState<ObjectEvidenceProvisionRequirement>::trampoline,
            holder.get(),
    ...
    std::future<Result<std::vector<ObjectAquisitionRequirement>>>
    dataProcessingDependencyReadRequirementAsync(const Query& request) {
        auto state = std::make_shared<StreamCollectState<ObjectAquisitionRequirement>>();
        auto future = state->promise.get_future();
        state->decoder = [](const pcl_msg_t* msg, ObjectAquisitionRequirement* out) {
            return decodeDataProcessingDependencyReadRequirementStreamFrame(msg, out);
        };
        auto holder = std::make_unique<StreamCollectHolder<ObjectAquisitionRequirement>>(StreamCollectHolder<ObjectAquisitionRequirement>{state});
        const pcl_status_t rc = invokeDataProcessingDependencyReadRequirementStream(
            executor_->handle(), request,
            &StreamCollectState<ObjectAquisitionRequirement>::trampoline,
            holder.get(),
            nullptr, nullptr, content_type_.c_str());
        if (rc == PCL_OK || rc == PCL_STREAMING) {
            (void)holder.release();

...
public:
    ProvidedComponent(pcl::Executor& executor,
        ...
        : pcl::Component("pyramid_services_tactical_objects_consumed_provider"),
          executor_(&executor),
          handler_(&handler),
          content_type_(std::move(content_type)),
          bridge_(*this) {}
...
public:
    ConsumedComponent(pcl::Executor& executor,
                      std::string content_type = kJsonContentType)
        : pcl::Component("pyramid_services_tactical_objects_consumed_consumer"),
          executor_(&executor),
          content_type_(std::move(content_type)) {}
    ...
    objectEvidenceReadDetailAsync(const Query& request) {
    ...
    std::future<Result<std::vector<ObjectEvidenceRequirement>>>
    objectSolutionEvidenceReadRequirementAsync(const Query& request) {
        auto state = std::make_shared<StreamCollectState<ObjectEvidenceRequirement>>();
        auto future = state->promise.get_future();
        state->decoder = [](const pcl_msg_t* msg, ObjectEvidenceRequirement* out) {
            return decodeObjectSolutionEvidenceReadRequirementStreamFrame(msg, out);
        };
        auto holder = std::make_unique<StreamCollectHolder<ObjectEvidenceRequirement>>(StreamCollectHolder<ObjectEvidenceRequirement>{state});
        const pcl_status_t rc = invokeObjectSolutionEvidenceReadRequirementStream(
            executor_->handle(), request,
            &StreamCollectState<ObjectEvidenceRequirement>::trampoline,
            holder.get(),
            nullptr, nullptr, content_type_.c_str());
        if (rc == PCL_OK || rc == PCL_STREAMING) {
            (void)holder.release();
    ...
    objectSourceCapabilityReadCapabilityAsync(const Query& request) {

                                               ...
                                               ObjectMatch* out)
{
    ...
    try {
        if (!is_json_content_type(msg->type_name)) {
            if (is_flatbuffers_content_type(msg->type_name)) {
                *out = flatbuffers_codec::fromBinaryObjectMatch(msg->data, msg->size);
                return true;
            }
            return false;
        }
        const std::string payload = msgToString(msg->data, msg->size);
        *out = fromJson(payload, static_cast<ObjectMatch*>(nullptr));
        return true;
    } catch (...) {
        return false;
    }
                                                      ...
                                                      ObjectInterestRequirement* out)
{
    ...
    try {
        if (!is_json_content_type(msg->type_name)) {
            if (is_flatbuffers_content_type(msg->type_name)) {
                *out = flatbuffers_codec::fromBinaryObjectInterestRequirement(msg->data, msg->size);
                return true;
            }
            return false;
        }
        const std::string payload = msgToString(msg->data, msg->size);
        *out = fromJson(payload, static_cast<ObjectInterestRequirement*>(nullptr));
        return true;
    } catch (...) {
        return false;
    }

...
is
   ...
   function To_Timeval (Value : Duration) return Timeval;
   --  Convert an Ada duration to a C timeval used by the SIGCHLD helper.
   ...
   function To_Timeval (Value : Duration) return Timeval is
      Whole_Seconds : constant Long_Long_Integer := Long_Long_Integer (Value);
      Seconds       : constant Duration := Duration (Whole_Seconds);
      Microseconds : constant Duration := (Value - Seconds) * 1_000_000.0;
   begin
      return
        (Tv_Sec  => C.long (Whole_Seconds),
         Tv_Usec => C.long (Long_Long_Integer (Microseconds)));
   end To_Timeval;
...
begin
   ...
   loop
      if Is_Infinite_Timeout then
         ...
         --  Wait
         declare
            Timeout_Value : aliased Timeval :=
              To_Timeval (End_Time - Cal.Clock);
         begin
            Status := Wait_For_Sigchld
              (Pipe_Read, Timeout_Value'Unchecked_Access);
         end;
      end if;

...
| **Interaction binding override** (tests/embedded use) | interaction facade (`configureInteractionBinding`) | `{"binding":"rpc"}` / `{"binding":"pubsub"}` or per-leg `{"request_leg":...,"entity_leg":...}`. Routed deployments infer this choice and should not duplicate it here. |

...
The pipeline tooling lives in `subprojects/PYRAMID/pim/` and is checked into this repository:

| File | Role |
|------|------|
| `subprojects/PYRAMID/pim/sysml_parser.py` | Parses Cameo/MagicDraw XMI export into JSON IR (`datamodel.json`) |
| `subprojects/PYRAMID/pim/proto_generator.py` | Generates `.proto` files from JSON IR with EntityActions CRUD services |
| `subprojects/PYRAMID/pim/cpp_model_generator.py` | Generates C++14 headers (unique_ptr, topo-sorted) from JSON IR |
| `subprojects/PYRAMID/pim/ada_model_generator.py` | Generates Ada type packages from JSON IR *(scaffolding: generated once, then manually owned)* |
| `subprojects/PYRAMID/pim/ada_service_generator.py` | Generates Ada service specs/stubs from `.proto` IDL (transport target — not a JSON IR consumer) |
| `subprojects/PYRAMID/pim/ada_type_generator.py` | Generates Ada type definitions |
| `subprojects/PYRAMID/pim/python_model_generator.py` | Generates Python dataclasses from JSON IR |
| `subprojects/PYRAMID/pim/contract_resolver.py` | Resolves provided/consumed service contracts between components |
| `subprojects/PYRAMID/pim/activity_parser.py` | Parses SysML activity diagrams |
| `subprojects/PYRAMID/pim/mock_generator.py` | Generates mock implementations for testing |
| `subprojects/PYRAMID/pim/test_generator.py` | Generates test harnesses |
...
Phase 2 — Generate .proto files for Tactical Objects component
          Check generated .proto into repo under subprojects/PYRAMID/proto/pyramid/
...
Issues in `subprojects/PYRAMID/pim/proto_generator.py`. Line references are to the current source as of 2026-03-15.
...
The code generation pipeline is checked into `subprojects/PYRAMID/pim/`. These are the codex generators that consume the JSON IR and produce target-language output.

| File | Input | Output | Notes |
|------|-------|--------|-------|
| `subprojects/PYRAMID/pim/sysml_parser.py` | XMI (Cameo export) | `datamodel.json` | Extracts DataTypes, Enumerations, Classes with properties, generalizations, flow direction |
| `subprojects/PYRAMID/pim/proto_generator.py` | `datamodel.json` | `.proto` files | EntityActions CRUD services; protobuf as IDL only (no gRPC runtime) |
| `subprojects/PYRAMID/pim/cpp_model_generator.py` | `datamodel.json` | C++14 headers | `unique_ptr`, topologically sorted, includes from generalizations |
| `subprojects/PYRAMID/pim/ada_model_generator.py` | `datamodel.json` | Ada type packages | Ada record types with discriminants |
| `subprojects/PYRAMID/pim/ada_type_generator.py` | `datamodel.json` | Ada type definitions | Low-level Ada type generation |
| `subprojects/PYRAMID/pim/ada_service_generator.py` | `.proto` file or directory | Ada service specs/stubs | Reads proto `service`/`rpc` blocks; transport target alongside ROS2/PCL |
| `subprojects/PYRAMID/pim/python_model_generator.py` | `datamodel.json` | Python dataclasses | Python model types |
| `subprojects/PYRAMID/pim/contract_resolver.py` | `datamodel.json` | Contract mappings | Resolves provided/consumed service contracts between components |
| `subprojects/PYRAMID/pim/activity_parser.py` | XMI | Activity models | Parses SysML activity diagrams for behavioural generation |
| `subprojects/PYRAMID/pim/mock_generator.py` | `datamodel.json` | Mock implementations | Test doubles for service interfaces |
| `subprojects/PYRAMID/pim/test_generator.py` | `datamodel.json` | Test harnesses | Generated test scaffolding |
...
The Ada service generator (`subprojects/PYRAMID/pim/ada_service_generator.py`) currently produces service stubs that bind to a `Pyramid.Middleware` abstraction — a low-level `Send`/`Receive`/`Receive_Any` interface over raw `System.Address` and integer channel IDs. This is a placeholder transport layer that assumes an unspecified middleware (DDS, ZeroMQ, shared memory, etc.).
...
Specific code changes in `subprojects/PYRAMID/pim/proto_generator.py`:
...
Key changes to `subprojects/PYRAMID/pim/ada_service_generator.py`:
...
1. Hand-write `.proto` files under `subprojects/PYRAMID/proto/pyramid/` based on:

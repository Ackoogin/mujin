...
class GrpcBackend(codec_backends.CodecBackend):
    ...
    def _write_cpp_plugin_aggregator_header(self, path: Path,
        ...
        cancel_fn = f'{symbol_prefix}_client_cancel_all'
        with open(path, 'w', encoding='utf-8', newline='\n') as f:
            ...
            f.write('    void*       user);\n\n')
            f.write('// Cancel every in-flight client RPC started by the invoke_* functions\n')
            f.write('// above. Used at plugin teardown so a wedged peer cannot strand the\n')
            f.write('// worker thread in a blocking call: TryCancel() makes the in-flight\n')
            f.write('// unary call / stream Read() return promptly with a cancelled status.\n')
            f.write(f'void {cancel_fn}(void);\n\n')
            f.write('#ifdef __cplusplus\n')
            f.write('}  // extern "C"\n')
            f.write('#endif\n')
    ...
    def _write_cpp_plugin_aggregator_impl(self, path: Path, entries: List[dict],
        ...
        with open(path, 'w', encoding='utf-8', newline='\n') as f:
            ...
            f.write('}\n\n')
            f.write('// Registry of in-flight client ClientContexts so plugin teardown can\n')
            f.write('// TryCancel() them (see the cancel_all entry below). Each invoke_*\n')
            f.write('// registers its stack ClientContext for the duration of the blocking\n')
            f.write('// call via ClientContextGuard.\n')
            f.write('std::mutex& clientContextMutex() {\n')
            f.write('    static std::mutex m;\n')
            f.write('    return m;\n')
            f.write('}\n\n')
            f.write('std::vector<grpc::ClientContext*>& activeClientContexts() {\n')
            f.write('    static std::vector<grpc::ClientContext*> contexts;\n')
            f.write('    return contexts;\n')
            f.write('}\n\n')
            f.write('struct ClientContextGuard {\n')
            f.write('    grpc::ClientContext* context;\n')
            f.write('    explicit ClientContextGuard(grpc::ClientContext* c) : context(c) {\n')
            f.write('        std::lock_guard<std::mutex> lock(clientContextMutex());\n')
            f.write('        activeClientContexts().push_back(context);\n')
            f.write('    }\n')
            f.write('    ~ClientContextGuard() {\n')
            f.write('        std::lock_guard<std::mutex> lock(clientContextMutex());\n')
            f.write('        auto& contexts = activeClientContexts();\n')
            f.write('        contexts.erase(\n')
            f.write('            std::remove(contexts.begin(), contexts.end(), context),\n')
            f.write('            contexts.end());\n')
            f.write('    }\n')
            f.write('    ClientContextGuard(const ClientContextGuard&) = delete;\n')
            f.write('    ClientContextGuard& operator=(const ClientContextGuard&) = delete;\n')
            f.write('};\n\n')
            f.write('}  // namespace\n\n')
            f.write('extern "C" {\n\n')
            f.write(f'{server_type}* {start_fn}(\n')
            ...
            for r in unary_rpcs:
                ...
                f.write('        grpc::ClientContext context;\n')
                f.write('        ClientContextGuard context_guard(&context);\n')
            ...
            f.write(f'void {cancel_fn}(void) {{\n')
            f.write('    std::lock_guard<std::mutex> lock(clientContextMutex());\n')
            f.write('    for (grpc::ClientContext* context : activeClientContexts()) {\n')
            f.write('        if (context) {\n')
            f.write('            context->TryCancel();\n')
            f.write('        }\n')
            f.write('    }\n')
            f.write('}\n\n')
            f.write('}  // extern "C"\n')

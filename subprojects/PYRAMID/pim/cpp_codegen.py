...
class CppServiceGenerator:
    ...
    def _write_components_header(self, path: Path, file_prefix: str,
        ...
        with open(path, 'w', encoding='utf-8', newline='\n') as f:
            ...
            f.write('    StreamHandle(StreamHandle&&) noexcept = default;\n')
            f.write('    StreamHandle& operator=(StreamHandle&&) noexcept = default;\n')
            ...
            for svc_name, rpc in all_rpcs:
                ...
                if rpc.streaming:
                    ...
                    f.write('        return StreamHandle{[push, ctx_handle]() {\n')
                    f.write('            push->cancelled = true;\n')
                    f.write('            if (ctx_handle) (void)pcl_stream_cancel(ctx_handle);\n')
                    f.write('        }};\n')
                    f.write('    }\n\n')
                else:
                    rsp_t = rpc.cpp_rsp_type

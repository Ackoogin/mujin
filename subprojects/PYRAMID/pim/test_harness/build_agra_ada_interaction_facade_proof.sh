#!/usr/bin/env bash
# Build + run the Ada interaction-facade runtime proof (WS-F/F1):
# agra_ada_interaction_facade_proof.adb exercises the generated
# MA_Action_Client_Bind/Submit_*/Transitions and
# MA_Action_Provider_Bind/Send_Transition surface end to end, under both
# the RPC and pub/sub realizations, in one process on one pcl::Executor
# (the Ada analogue of test_pcl_generated_interaction_facade.cpp's scope --
# not the cross-process real-transport proof agra_seam_interchange_test.cpp
# is for C++; see doc/todo/PYRAMID/TODO.md WS-F F1 for what's covered).
#
...
pyramid="$(cd "$here/../.." && pwd)"
root="$(cd "$pyramid/../.." && pwd)"
gen="$here/generated_agra_ada_interaction_facade_proof"

command -v gnatmake >/dev/null 2>&1 || {
  echo "gnatmake not found on PATH -- install GNAT to run this proof."
  exit 2
}
...
echo "== compiling + linking the Ada proof =="
gnatmake -q -gnat2020 -I"$gen" -I"$pcl_ada" \
    -o "$here/agra_ada_interaction_facade_proof" \
    "$here/agra_ada_interaction_facade_proof.adb" \
    -largs "${marshal_objs[@]}" "$libpcl" -ldl -lpthread -lstdc++

echo "== running =="
"$here/agra_ada_interaction_facade_proof" \

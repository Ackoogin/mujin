@echo off
REM build_agra_mixed_route_test.bat -- Windows counterpart to
REM build_agra_mixed_route_test.sh: Phase E (plan-terminal) of
REM doc/plans/PYRAMID/agra_pubsub_shm_udp_proving_plan.md. Authored for
REM parity; not exercised in this Linux environment (carried note, same as
REM the other test_harness .bat scripts).
setlocal enabledelayedexpansion

set "HERE=%~dp0"
for %%I in ("%HERE%..\..") do set "PYRAMID=%%~fI"
for %%I in ("%PYRAMID%\..\..") do set "ROOT=%%~fI"
set "GEN=%HERE%generated_agra_mixed_route"
set "SCRATCH=%HERE%agra_mixed_route_scratch"
...
set SRCS="%HERE%agra_mixed_route_test.cpp" "%GEN%\pyramid_services_agra_mission_autonomy_provided.cpp" "%GEN%\pyramid_services_agra_c2_station_consumed.cpp"
...
echo == compiling ==
cl /nologo /std:c++17 /EHsc /MD %INC% %SRCS% "%LIBPCL%" /Fe:"%HERE%agra_mixed_route_test.exe" /Fo"%GEN%\\"
if errorlevel 1 exit /b 1

echo == running ==
"%HERE%agra_mixed_route_test.exe" "%SHM_PLUGIN%" "%UDP_PLUGIN%" ^

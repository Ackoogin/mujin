#!/usr/bin/env bash
# Build + run Phase E (plan-terminal) of
# doc/plans/PYRAMID/agra_pubsub_shm_udp_proving_plan.md: MissionAutonomy and
# C2Station each load two transports (real SHM + real UDP) from one routing
# manifest and run the complete worked-example sequence -- action tasking
# and status over the SHM correlated pair, plan publication over UDP,
# cancel, all sequence checks, both codecs, plus a replay of the Phase D
# negative gate.
set -eu
...
pyramid="$(cd "$here/../.." && pwd)"
root="$(cd "$pyramid/../.." && pwd)"
gen="$here/generated_agra_mixed_route"
scratch="$here/agra_mixed_route_scratch"
...
srcs=("$here/agra_mixed_route_test.cpp"
...
g++ -std=c++17 "${inc[@]}" "${srcs[@]}" "$libpcl" -ldl -lpthread \
    -o "$here/agra_mixed_route_test"

echo "== running =="
"$here/agra_mixed_route_test" "$shm_plugin" "$udp_plugin" \

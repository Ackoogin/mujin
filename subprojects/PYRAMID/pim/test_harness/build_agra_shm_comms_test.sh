...
# codec first, FlatBuffers second (§4 Phase C).
#
...
# loadable .so and dlopen'd per-process (mirroring the FlatBuffers-as-.so
# pattern build_comms_test.sh already uses, just applied to JSON too).
set -eu

here="$(cd "$(dirname "$0")" && pwd)"
pyramid="$(cd "$here/../.." && pwd)"
root="$(cd "$pyramid/../.." && pwd)"
gen="$here/generated_agra_shm_comms"
scratch="$here/agra_shm_comms_scratch"
...
build_role() {
  ...
  g++ -std=c++17 -fPIC -shared "${inc[@]}" "${flat_plug[@]}" \
      -o "$here/lib${role_label}_agra_flatbuffers_codec.so"
}
...
# Main test binary: the harness + both packages' typed wrapper/cabi-marshal
# sources (needed to translate C++ structs before/after codec dispatch --
# the codec plugins themselves stay dlopen'd .so's, never linked here).
...
srcs=("$here/agra_shm_comms_test.cpp"
...
g++ -std=c++17 "${inc[@]}" "${srcs[@]}" "$libpcl" -ldl -lpthread \
    -o "$here/agra_shm_comms_test"

echo "== running =="
"$here/agra_shm_comms_test" "$shm_plugin" \
    "$here/libma_agra_json_codec.so" "$here/libc2_agra_json_codec.so" \
    "$here/libma_agra_flatbuffers_codec.so" "$here/libc2_agra_flatbuffers_codec.so" \
    "$scratch"

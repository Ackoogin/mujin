...
if "%OUT_DIR%"=="" set "OUT_DIR=%BUILD_DIR%\ada_gnat_pyramid_cabi"
...
if not defined GXX_DIR (
  echo [ada-cabi] ERROR: g++ not found in PATH
  exit /b 1
)
...
if not defined SOURCE_FILES (
  echo [ada-cabi] ERROR: no generated C-ABI marshal sources found
  if exist "%TMP_MANIFEST_FILE%" del /f /q "%TMP_MANIFEST_FILE%" >nul 2>&1
  exit /b 1
)

...
REM Prefer the OMS codec belonging to the requested formal seam over any
REM unrelated codec left in a shared build directory.
if "%GRA%"=="1" if /i not "%PROTO_DIR:agra_p3_seam=%"=="%PROTO_DIR%" if exist "%BUILD_DIR%\subprojects\PYRAMID\Release\pyramid_codec_oms_json_agra_core_mms.dll" set "OMS_CODEC_DLL=%BUILD_DIR%\subprojects\PYRAMID\Release\pyramid_codec_oms_json_agra_core_mms.dll"
if "%GRA%"=="1" if /i not "%PROTO_DIR:agra_p2_seam=%"=="%PROTO_DIR%" if exist "%BUILD_DIR%\subprojects\PYRAMID\Release\pyramid_codec_oms_json_agra.dll" set "OMS_CODEC_DLL=%BUILD_DIR%\subprojects\PYRAMID\Release\pyramid_codec_oms_json_agra.dll"
...
if not exist "%OUT_DIR%\generator\cpp\component_skeleton_gen.py" (
  echo [package_sdk] FAIL: missing C++ component skeleton generator
  exit /b 1
)
if not exist "%OUT_DIR%\generator\ada\component_skeleton_gen.py" (
  echo [package_sdk] FAIL: missing Ada component skeleton generator
  exit /b 1
)

...
if /i "%~1"=="--transport-plugin" (set "TRANSPORT_PLUGIN=%~2" & shift & shift & goto parse_args)
if /i "%~1"=="--codec-plugin" (set "CODEC_PLUGIN=%~2" & shift & shift & goto parse_args)
...
if /i "%~1"=="-TransportPlugin" (set "TRANSPORT_PLUGIN=%~2" & shift & shift & goto parse_args)
if /i "%~1"=="-CodecPlugin" (set "CODEC_PLUGIN=%~2" & shift & shift & goto parse_args)
if /i "%~1"=="-Address" (set "ADDRESS=%~2" & shift & shift & goto parse_args)
if /i "%~1"=="-Timeout" (set "TIMEOUT_SEC=%~2" & shift & shift & goto parse_args)
...
if "%TRANSPORT_PLUGIN%"=="" set "TRANSPORT_PLUGIN=%BUILD_DIR%\subprojects\PYRAMID\tests\%BUILD_CONFIG%\pyramid_grpc_coupled_plugin.dll"
...
if %errorlevel% equ 0 (
    ...
    pushd "!ADA_DIR!"
    gprbuild -P ada_grpc_cpp_interop_e2e.gpr -q -XUNMANNED_ROOT="%WORKSPACE_ROOT%" -XPCL_LIB_DIR="%BUILD_DIR%\ada_gnat_pcl" -XPYRAMID_CABI_LIB_DIR="%BUILD_DIR%\ada_gnat_pyramid_cabi" >nul 2>&1
    if !errorlevel! neq 0 (
...
if not exist "%TRANSPORT_PLUGIN%" (
    echo [driver] FAIL: gRPC transport plugin not found at %TRANSPORT_PLUGIN%
    exit /b 1
)
if "%CODEC_PLUGIN%"=="" (
    echo [driver] FAIL: missing --codec-plugin
    exit /b 1
)
if not exist "%CODEC_PLUGIN%" (
    echo [driver] FAIL: protobuf codec plugin not found at %CODEC_PLUGIN%
    exit /b 1
)
...
"%CLIENT_BIN%" --transport-plugin "%TRANSPORT_PLUGIN%" --codec-plugin "%CODEC_PLUGIN%" --address "%ADDRESS%"

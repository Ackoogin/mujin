...
echo [driver] bus=%BUS_NAME% port=%PORT% timeout=%TIMEOUT_SEC%s
...
if %errorlevel% equ 0 (
    ...
    call "%PYRAMID_ROOT%\scripts\build_gnat_generated_flatbuffers_libs.bat" "!ADA_PYRAMID_LIB_DIR!" --force
    if !errorlevel! neq 0 (
...
(
    echo @echo off
    echo "%STUB_BIN%" --bus "%BUS_NAME%" --timeout %TIMEOUT_SEC%
    echo echo %%ERRORLEVEL%% ^> "%RC_FILE%"
) > "%STUB_WRAPPER%"
...
start /b "" "%APP_BIN%" --port %PORT% --port-file "%PORT_FILE%" --timeout %TIMEOUT_SEC% --demo-evidence >nul 2>&1
...
start /b "" "%BRIDGE_BIN%" --tobj-host 127.0.0.1 --tobj-port %ACTUAL_PORT% --ame-bus "%BUS_NAME%" --timeout %TIMEOUT_SEC% >nul 2>&1
